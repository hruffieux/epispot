...
#
epispot_probit_core_ <- function(Y, X, Z, list_hyper, gam_vb, mu_alpha_vb,
  ...
  with(list_hyper, { # list_init not used with the with() function to avoid
    ...
    if (full_output) { # for internal use only
      ...
      create_named_list_(gam_vb, om_vb, mu_alpha_vb, mu_beta_vb, converged, it, lb_opt, diff_lb)
    }
  })
...
# lower bound (ELBO) corresponding to the `epispot_probit_core` algorithm.
#
