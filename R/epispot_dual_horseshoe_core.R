...
#
epispot_dual_horseshoe_core_ <- function(Y, X, list_hyper, gam_vb, mu_beta_vb, 
  ...
  with(list_hyper, { # list_init not used with the with() function to avoid
    ...
    while ((!converged) & (it < maxit)) {
      ...
      W <- update_W_info_(gam_vb, theta_plus_rho_vb, log_1_min_Phi_theta_plus_rho, 
                          log_Phi_theta_plus_rho, c = c) # we use info_ so that the second argument is a matrix
      ...
      if (annealing) {
        ...
        lb_new <- elbo_dual_horseshoe_(Y, a_inv_vb, A2_inv, b_vb, eta, eta_vb, G_vb, gam_vb, kappa, kappa_vb, lambda,
                                       lambda_vb, lambda_a_inv_vb, lambda_s0_vb, log_1_min_Phi_theta_plus_rho, 
                                       log_Phi_theta_plus_rho, m0, n0, mu_rho_vb,
...
elbo_dual_horseshoe_ <- function(Y, a_inv_vb, A2_inv, b_vb, eta, eta_vb, G_vb, 
                                 ...
                                 lambda_a_inv_vb, lambda_s0_vb, log_1_min_Phi_theta_plus_rho, 
                                 log_Phi_theta_plus_rho, m0, n0, mu_rho_vb,
  ...
  if (is.null(list_struct)) {
    elbo_B <- e_beta_gamma_dual_(gam_vb, log_sig2_inv_vb, log_tau_vb,
                                 log_1_min_Phi_theta_plus_rho, 
                                 log_Phi_theta_plus_rho, m2_beta,
    ...
    elbo_B <- sum(sapply(1:n_bl, function(bl) {
      e_beta_gamma_dual_(gam_vb[vec_fac_bl == bl_ids[bl], , drop = FALSE], log_sig2_inv_vb, log_tau_vb,
                         log_1_min_Phi_theta_plus_rho[vec_fac_bl == bl_ids[bl], , drop = FALSE], 
                         log_Phi_theta_plus_rho[vec_fac_bl == bl_ids[bl], , drop = FALSE], 
