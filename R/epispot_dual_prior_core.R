...
#
epispot_dual_prior_core_ <- function(Y, X, list_hyper, gam_vb, mu_beta_vb, sig2_beta_vb,
  ...
  with(list_hyper, { # list_init not used with the with() function to avoid
    ...
    while ((!converged) & (it < maxit)) {
      ...
      W <- update_W_info_(gam_vb, theta_plus_rho_vb, 
                          log_1_min_Phi_theta_plus_rho, log_Phi_theta_plus_rho, c = c) # we use info_ so that the second argument is a matrix
      ...
      if (annealing) {
        ...
        lb_new <- elbo_dual_prior_(Y, eta, eta_vb, gam_vb, kappa, kappa_vb, lambda,
                                   lambda_vb, lambda_s0, lambda_s0_vb, 
