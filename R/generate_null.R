...
#'
#' This function runs \code{\link{epispot}} on data with permuted response sample
...
#'
#' @param n_perm Number of permuted datasets on which \code{\link{epispot}} is
#'   run.
...
#'   with each element corresponding to the output of \code{\link{epispot}} on one
...
#
#' @seealso @seealso \code{\link{epispot}}, \code{\link{set_hyper}},
...
generate_null <- function(n_perm, Y, X, p0_av, Z = NULL, link = "identity",
  ...
  permute <- function(i) {
    ...
    res_perm <- epispot(Y = Y[ind_perm, ], X = X, p0_av = p0_av, Z = Z,
